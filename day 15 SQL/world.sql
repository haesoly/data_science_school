...
# 각 컬럼의 이름을 재정의 가능
select Name as n, Population as p 
	from city;
...
# Where 조건절
# Select에서 특정한 조건을 만족하는 열만 가져올 수 있음
select * 
	from city 
...
# Index
# 서적의 인덱스와 동일한 기능
# 빠른 검색을 위한 추가적인 데이터 구조
# 검색은 빨라지나, 추가, 변경, 삭제는 상대적으로 느려짐
# 이유는, 인덱스를 다시 만들어야 하기 때문
...
# 인덱스 제거
drop index city_idx1 on city;

# 쿼리의 실행에 대한 기본적인 파악 가능
explain select * 
	from city 
...
create index city_idx2 on city(CountryCode, Population);
drop index city_idx2 on city;
...
# Group by
# 같은 조건을 갖는 값끼리 grouping 가능
# mysql 5.7 부터, 기본적으로 group by의 대표값을 못참음 
# 아래와 같이 해결 
...
# 위의 결과 중 한국의 결과만 뽑아보세요

# 먼저 조건을 만족하는 데이터를 가져와서 그루핑
select CountryCode, sum(Population)
	from city
    where CountryCode = 'KOR'
    group by CountryCode;
...
# 컨츄리코드 별로의 인구 합이 3000만이 넘는 도시를 구하시오
select CountryCode, sum(Population)
	from city 
    group by CountryCode
...
# 아래와 같이 할 수 없다. 
# Where의 경우, 각각의 row를 기준으로 하기 때문에 group function을 사용할 수 없다.
select CountryCode, sum(Population)
	from city 
    where sum(Population) > 3000000
    group by CountryCode;
...
select Name, max(Population) as m
	from city
    group by Name
    order by m desc;
...
select CountryCode, sum(Population) as total_pop
	from city
    group by CountryCode
    order by total_pop limit 1;

	
# subquery
# query의 where 절이나, from 절에 query를 사용
...
# 평균보다 높은 인구수를 갖는 도시 출력

select avg(Population) 
	from city;
    
select * from city where Population > 34343;    
...
select ID, Name, District from 
	(select *
		from city
		where Population > 3000000) t
	order by ID;
...
select * 
	from city 
    where Population > 1000000) t # 임시 테이블 명을 명시해야함
    where t.CountryCode = 'KOR';
...
# 상관 서브 쿼리는 효율적이지 않다. 
# 1. outer query의 row 한개를 선택한다.
# 2. 해당 row의 값으로 inner query를 실행
# 3. 2에서 실행된 결과로 where문을 평가하여 row를 선택할지 말지 결정한다. 
# 4. 모든 outer table의 row에 대해서 1 - 3 반복
        
# 추후 join을 배운 뒤, 더 효율적으로 작성 가능



# join
# 테이블을 서로 연결하여 검색에 사용하는 방법
# 교차조인 
# inner 조인 
# outer 조인
